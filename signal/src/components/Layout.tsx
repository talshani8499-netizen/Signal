      ...
      <div className="sticky top-0 z-50 pt-4 pb-4 px-4 sm:px-6 w-full max-w-7xl mx-auto">
        <header className="h-16 px-6 flex items-center justify-between bg-warm-bg/90 backdrop-blur-xl border border-ink/10 rounded-2xl shadow-sm">
          ...
          <nav className="flex items-center gap-6">
            {(location.pathname === '/' || location.pathname === '/v2') && (
              <Link 
                to={location.pathname === '/' ? '/v2' : '/'} 
                className="text-xs font-bold uppercase tracking-wider bg-lilac/20 text-lilac-dark px-3 py-1.5 rounded-full hover:bg-lilac/30 transition-colors border border-lilac/30"
              >
                {location.pathname === '/' ? 'View V2' : 'View V1'}
              </Link>
            )}
            <Link 
              to="/dashboard" 
